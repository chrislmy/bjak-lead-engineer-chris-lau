# Personal work-experience assistant — implementation plan

Time box: ~120 minutes. Goal: a thin, grounded, evaluable slice — not a complete system.
...
## Still open (does not block M1–M7)
...
- `src/cli.ts` — usage only (`ingest` / `ask` / `eval` stub)
- `src/env.ts` — read `OPENAI_BASE_URL`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `JUDGE_MODEL`
...
## M2 — Placeholder knowledge sources
...
- `fixtures/cv.md` — placeholder Coda / ClickHouse / Data Hub / Senior title
- `fixtures/linkedin.md` — Lead title + one extra bullet + synthetic label
...
- `src/sources/cv.ts`, `src/sources/linkedin.ts` — each `ingest()` + `retrieve()` (retrieve can wait until M4 if you prefer, but the module should exist)
- `src/sources/index.ts` — `ingestAll()`
...
- [ ] Adding a third source is “new file + one array entry” (stated in a code comment or README)
...
- `retrieveAll()` — both full docs; **do not use the query**
- `src/llm/client.ts` — OpenAI SDK, env `baseURL` / `apiKey` / `model`
- `src/assistant.ts` — prompt: stay in supplied context; refuse if insufficient; surface disagreements
...
- `eval/golden.json` — 25 cases (8 direct, 4 multi_source, 4 ambiguous, 4 unanswerable, 5 adversarial)
- `eval/golden.ts` — types + `contractText(g)` building `BEHAVIOUR` / `MUST CONTAIN` / `MUST NOT CLAIM`
...
- [ ] Refusal suite is defined as `behaviour === "refuse"` (not “all adversarial”)
...
## M6 — LabelContract + Refusal eval (required)
...
- `eval/geval.ts` — `OllamaModel` judge `llama3.1:8b`; `labelContract`; `refusalInjection`
- `eval/assistant.eval.ts` — `it.each` all goldens ? LabelContract; filter `behaviour === "refuse"` ? RefusalInjection
- `vitest.config.ts` — long `testTimeout` / `hookTimeout`
- `eval/results/` — last run output (table or DeepEval report) plus **two** failure write-ups
...
- [ ] Two real failures explained (not “will fail later”)
...
- `ingest` / `ask` / `eval` (`eval` may shell `npx deepeval test run …`)
...
## M8 — Leadership write-up and submission
...
- `docs/decisions.md` (or README section): 3–5 ADRs (full-doc vs query retrieve; OpenAI SDK + Ollama; conflict policy; GEval vs custom matcher; skip YAML-as-source)
- Cuts + next three
...
Never cut: M2–M4 behaviours (sources, refuse, conflict), M5 count/categories, M6 run + two failures, M8 README + time spent.
